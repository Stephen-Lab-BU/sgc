...
from cohlib.utils import pickle_save, logistic
...
def run():
    ...
    print(f"Generating Synthetic SGC data (no DC) with L: {L}, K: {K}, sample_length: {sample_length}, C: {C}, alpha: {alpha}, seed: {seed}")
    ...
    # lams = np.apply_along_axis(cif_alpha, 2, xs)
...
def construct_latent_and_sample_bcn_mod(sample_length, L, fs, K):
    T = sample_length/fs
    Gamma, freqs = gen_random_mvcn_params(T, fs, K)
    n_freqs = Gamma.shape[0]
