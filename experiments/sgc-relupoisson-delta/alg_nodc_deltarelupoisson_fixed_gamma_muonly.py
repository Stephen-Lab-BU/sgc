...
def run():
    ...
    parser.add_argument('mu_only_update', nargs='?', type=bool, default=False)
    ...
    mu_only = args.mu_only_update

    if args.rho == 0:
        rho = None
        kappa = None
        if mu_only is True:
            print(f"Fitting poisson data with ReLU link; mu-only update. L: {L}, K: {K}, sample_length: {sample_length}, C: {C}, alpha: {alpha}, seed: {seed}")
        else:
    ...
    if mu_only is True:
        save_path = f'saved/fitted_models/simple_synthetic_deltarelupoisson_em{num_em}_{K}_{L}_{sample_length}_{C}_{alpha}_{seed}_{init_type}_{optim_type}_mu_only_fitted'
    else:
