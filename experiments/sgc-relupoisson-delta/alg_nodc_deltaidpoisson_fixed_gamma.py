...
def run():
    ...
    if args.rho == 0:
        rho = None
        kappa = None
        print(f"Fitting poisson data with ID link, L: {L}, K: {K}, sample_length: {sample_length}, C: {C}, alpha: {alpha}, seed: {seed}")
    else: 
        rho = args.rho
        kappa = args.kappa
        print(f"Fitting (regularized) poisson data ID link. Params - rho: {rho}, kappa: {kappa}, L: {L}, K: {K}, sample_length: {sample_length}, C: {C}, alpha: {alpha}, seed: {seed}")
    ...
    save_path = f'saved/fitted_models/simple_synthetic_deltaidpoisson_em{num_em}_{K}_{L}_{sample_length}_{C}_{alpha}_{seed}_{init_type}_{optim_type}_fitted'
