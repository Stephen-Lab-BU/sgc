...
def run():
    ...
    data_path = f'saved/synthetic_data/singlefreq_{J_sel}_deltarelu_poisson_{K}_{L}_{sample_length}'
    save_path = f'saved/fitted_models/singlefreq_{J_sel}_synthetic_deltarelupoisson_em{num_em}_{K}_{L}_{sample_length}_{C}_{alpha}_{seed}_{init_type}_{optim_type}_fitted'
    ...
    Gamma_sampletrue_inv = np.stack([np.linalg.inv(Gamma_est_z[j,:,:]) for j in range(Gamma_est_z.shape[0])])
