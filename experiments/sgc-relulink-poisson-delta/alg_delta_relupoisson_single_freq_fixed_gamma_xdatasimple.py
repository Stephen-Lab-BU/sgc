...
def run():
    ...
    if args.rho == 0:
        rho = None
        kappa = None
        print(f"Fitting single-freq ({J_sel} Hz) poisson data with ReLU link, L: {L}, K: {K}, sample_length: {sample_length}, C: {C}, alpha: {alpha}, seed: {seed}")
    else: 
        rho = args.rho
        kappa = args.kappa
    ...
    orig_data_path = f'saved/synthetic_data/singlefreq_{J_sel}_latent_deltarelu_{K}_{L}_{sample_length}_{seed}.pkl'
    save_path = f'saved/fitted_models/crossdata_singlefreq_{J_sel}_deltarelu_poisson_em{num_em}_{K}_{L}_{sample_length}_{C}_{alpha}_{seed}_{init_type}_{optim_type}_fitted.pkl'
    ...
    save_dict = load_and_fit_model(Wv, data_load, C, alpha, init_type, optim_type, rho, kappa, store_spikes, num_em, orig_data=orig_data_load)
