...
def run():
    ...
    J_sel = 10
    data_path = f'saved/synthetic_data/singlefreq_{J_sel}_latent_deltarelu_{K}_{L}_{sample_length}_{seed}.pkl'
    orig_data_path = f'saved/synthetic_data/simple_latent_deltarelu_{K}_{L}_{sample_length}_{seed}.pkl'
    save_path = f'saved/fitted_models/cross_data_simple_deltarelu_poisson_em{num_em}_{K}_{L}_{sample_length}_{C}_{alpha}_{seed}_{init_type}_{optim_type}_fitted.pkl'

    # data_load = pickle_open(load_path)
    data_load = pickle_open(data_path)
    orig_data_load = pickle_open(orig_data_path)
    freqs = orig_data_load['meta']['freqs']

    J_orig = int(sample_length / 2)
    J_new = np.where(freqs > 50)[0][0] - 1
    Wv = construct_real_idft_mod(sample_length, J_orig, J_new, fs)

    save_dict = load_and_fit_model(Wv, data_load, C, alpha, init_type, optim_type, rho, kappa, store_spikes, num_em, J_model=J_new, orig_data=orig_data_load)
