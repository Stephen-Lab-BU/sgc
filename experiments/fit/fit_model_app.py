...
from cohlib.confs.config import get_fit_config, get_app_config
...
def run(cfg: Config) -> None:
    ...
    app_dir = conf.get_app_dir(acfg)
    latent_dir = conf.get_app_latent_dir(lcfg, app_dir)
    obs_dir = conf.get_app_obs_dir(ocfg, latent_dir)
    model_subdir = conf.get_model_subdir(mcfg)
    model_dir = os.path.join(obs_dir, model_subdir)

    # use this to specify data to load, and in future any formatting / preprocessing
