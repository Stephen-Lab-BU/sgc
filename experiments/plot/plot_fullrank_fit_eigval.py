...
from cohlib.confs.model import FullRankToySimple
...
def plot(cfg: Config):
    ...
    for l, L in enumerate(Ls):
        for t, theta in enumerate(thetas):
            ...
            plot_eigval_var_L_subplot(ax[t,l], plot_data[t,l], drop_emp=False)
...
def get_plot_dir(cfg, eigrank):
    plot_dir = f'data/figs-fit/{cfg.plot.plot_type}/latent-{cfg.latent.latent_type}/fullrank/window-{int(2*cfg.latent.num_freqs)}/K{cfg.latent.K}/obs-{cfg.obs.obs_type}/init-{cfg.model.model_init}/eigval{eigrank}'
    return plot_dir
