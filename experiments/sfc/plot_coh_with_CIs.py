...
def run():
    ...
    parser.add_argument('snu', nargs='?', type=float, default=0.1)
    ...
    print(f"Plotting Coherence for fitted model - synthetic data with L: {L}, sample_length: {sample_length}, C: {C}, seed: {seed}, mu: {mu}, sigma_nu: {snu}")

    load_data_path = f'saved/synthetic_data/simple_synthetic_{L}_{sample_length}_{C}_{mu}_{snu}_{seed}'
    # save_path = f'saved/naive_ests/simple_synthetic_{L}_{sample_length}_{C}_{mu}_{snu}_{seed}'

    load_res_path = f'saved/fitted_models/simple_synthetic_{L}_{sample_length}_{C}_{mu}_{snu}_{seed}'


    data_load = pickle_open(load_data_path)
    freqs = data_load['meta']['freqs']

    res_load = pickle_open(load_res_path)
    z_ests_lfp = res_load['lfp']['z_ests']

    v_ests = res_load['spikes']['v_ests']
    cnss = []
    for l in range(L):
    ...
    zs_est = np.stack([z_ests_spikes, z_ests_lfp],axis=1)

    J = z_ests_lfp.shape[1]
    ...
    save_path = f'saved/figures/simple_synthetic_{L}_{sample_length}_{C}_{mu}_{snu}_{seed}.png'
