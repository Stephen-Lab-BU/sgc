...
def run():
    ...
    print(f"Fitting SFC model for synthetic data with L: {L}, sample_length: {sample_length}, C: {C}, seed: {seed}, mu: {mu}, sigma_nu: {snu}")

    load_path = f'saved/synthetic_data/simple_synthetic_{L}_{sample_length}_{C}_{mu}_{snu}_{seed}'
    save_path = f'saved/fitted_models/simple_synthetic_{L}_{sample_length}_{C}_{mu}_{snu}_{seed}'
    ...
    v_ests_spikes, var_ests_spikes, track_spikes = fit_model(spikes, 
                                        W, 
                                        params_init, 
                                        n_iter=n_iter, 
                                        EM=True, 
    ...
    spikes_save_dict = dict(v_ests=v_ests_spikes, var_ests=var_ests_spikes, sigma=sigma_est, seps=seps_est)

    save_dict = dict(lfp=lfp_save_dict, spikes=spikes_save_dict)
