...
def run():
    ...
    print(f"Generating Synthetic SFC data with L: {L}, sample_length: {sample_length}, C: {C}, mu: {mu}, sigma_nu: {snu}, seed: {seed}")

    save_path = f'saved/synthetic_data/simple_synthetic_{L}_{sample_length}_{C}_{mu}_{snu}_{seed}'

    # turn into argparse if need to manipulate
    fs = 1000
    # mu = -3.5
    # beta = 2.75
    beta = 1
    ...
    spikes = spikes_from_xns(xns, pp_params, L, sample_length)
    lfp = xys + np.sqrt(snu)*np.random.randn(L, sample_length)

    observed = dict(lfp=lfp, spikes=spikes, snu=snu, pp_params=pp_params, mu=mu, beta=beta)
...
def construct_latent_distribution(sample_length, L, fs, norm='backward'):
    T = sample_length/fs
