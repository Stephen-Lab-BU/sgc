...
def run():
    ...
    save_path = '/projectnb/stephenlab/jtauber/cohlib/hydra/refac_pprelu_obs/figs/rank1naive'
    if init_use == 'all':
        init_use = 'flat-init'
...
def plot_eigvals_var_L(res_load, naive_ests, oracle_ests, mus, Ls, K, jind, eigrank, Nnz, savename=None):
    ...
    fig, ax = plt.subplots(mus.size, Ls.size, figsize=(12,8), sharex=True, sharey='row')
    jind_full=9
    jind=0
...
def get_naive_and_oracle(res_load, Ls, mus, K, Nnz):
    ...
    gamma_name = f'k{K}-chlg3-relu-rank1-nz9'
    gamma_name_old = f'k{K}-chlg3-rank1-nz9'
    for l, L in enumerate(Ls):
        for a, mu in enumerate(mus):
            print(f'L: {L},  mu: {mu}' )
            lsel = {'L': L,
            'gamma': gamma_name_old}
            msel = {}
            osel = {'alpha': mu}
            res = filter_loaded(res_load, lsel, msel, osel)
