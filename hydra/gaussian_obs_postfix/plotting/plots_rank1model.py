...
def run():
    ...
    res_paths = [f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/outputs/{date}-{ts_flag}-{ts_flag2}-{addendum}/{gamma_name}/seed-{seed}/{m_step_init}']
    save_dir_gamma = f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/figs/local_r1new/{ts_flag}-{addendum}/{ts_flag2}/{gamma_name}/{m_step_init}'
    ...
    for init in inits:
        ...
        savename = os.path.join(save_dir_init, 'eigval1-'+name)
        ...
        for dim in dims:
            savename = os.path.join(save_dir_init, f'eigvec-phase-dim{dim}-'+name)
            ...
            savename = os.path.join(save_dir_init, f'eigvec-mag-dim{dim}-'+name)
            ...
            savename = os.path.join(save_dir_init, f'eigvec-real-dim{dim}-'+name)
            ...
            savename = os.path.join(save_dir_init, f'eigvec-imag-dim{dim}-'+name)
