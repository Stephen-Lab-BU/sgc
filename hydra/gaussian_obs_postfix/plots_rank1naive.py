...
def run():
    ...
    parser.add_argument('num_em', nargs='?', type=int, default=50)
    ...
    res_paths = ['/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/batch/outputs/2024-11-25-ch4prelim']
    ...
    save_path = '/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/figs'
    if init_use == 'all':
        init_use = 'flat-init'
        res_load = load_res_for_plot(init_use, seed, res_paths, num_em_iters)
        naive_ests, oracle_ests = get_naive_and_oracle(res_load, Ls, ov_pairs, K, Nnz)

        name = f'k{K}-{init_use}-seed{seed}_var_L_gaussian_12_10_2024.png'
        savename = os.path.join(save_path, name)
        plot_eigvals_var_L(res_load, naive_ests, oracle_ests, ov_pairs, Ls, K, jind, eigrank, Nnz, savename=savename)
...
def plot_eigvals_var_L(res_load, naive_ests, oracle_ests, ov_pairs, Ls, K, jind, eigrank, Nnz, savename=None):
...
def get_naive_and_oracle(res_load, Ls, ov_pairs, K, Nnz):
    ...
    for l, L in enumerate(Ls):
        for a, ov_pair in enumerate(ov_pairs):
            ...
            gamma_full_dummytarget = jnp.zeros((num_freqs, K, K), dtype=complex)
