...
def run():
    ...
    # res_paths = [f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/outputs/2024-12-17-fixed_eigval_true/{gamma_name}/seed-{seed}']
    # print(res_paths)
    # save_dir_gamma = f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/figs/local_r1new/fixed-eigval-true/{gamma_name}/'

    # res_paths = [f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/batch/outputs/2024-12-17-fixed_u_true/{gamma_name}/seed-{seed}']
    # save_dir_gamma = f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/figs/batch_r1m_full/fixed-u-true/{gamma_name}/seed-{seed}/'

    res_paths = [f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/batch/outputs/2024-12-17-fixed_eigval_true/{gamma_name}/seed-{seed}']
    save_dir_gamma = f'/projectnb/stephenlab/jtauber/cohlib/hydra/gaussian_obs_postfix/figs/batch_r1m_full/fixed-eigval-true/{gamma_name}/seed-{seed}/'
    ...
    for init in inits:
        ...
        save_dir_seed = os.path.join(save_dir_init, f'seed-{seed}')
        if not os.path.exists(save_dir_seed):
            os.makedirs(save_dir_seed)

        savename = os.path.join(save_dir_seed, 'eigval1-'+name)
        ...
        for dim in dims:
            savename = os.path.join(save_dir_seed, f'eigvec-phase-dim{dim}-'+name)
            ...
            savename = os.path.join(save_dir_seed, f'eigvec-mag-dim{dim}-'+name)
            ...
            savename = os.path.join(save_dir_seed, f'eigvec-real-dim{dim}-'+name)
            ...
            savename = os.path.join(save_dir_seed, f'eigvec-imag-dim{dim}-'+name)
...
def plot_func_eigvec_var_L(func, funcname, dim, res_load, naive_ests, oracle_ests, ov_pairs, Ls, K, jind, eigrank, Nnz, gamma_name, savename=None):
    ...
    for l, L in enumerate(Ls):
        for a, ov_pair in enumerate(ov_pairs):
            ...
            eigvec_true = res['eigvecs_true'].squeeze()
            eigvecs_em = jnp.stack([x.eigvecs.squeeze() for x in res['track']['gamma_lowrank']])
