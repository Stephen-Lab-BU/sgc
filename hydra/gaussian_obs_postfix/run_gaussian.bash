    ...
    do 
        ...
        do
            ...
            do
                ...
                # model.ts_flag2=gd model.ts_flag=$ts_flag model.emiters=$iters \
                # model.m_step_init=$m_step_init model.init=$init model.scale_init=$scale_init \
                # obs.ov1=$ov1 obs.ov2=$ov2 \
                # hydra.job.chdir=True hydra.run.dir='outputs/${now:%Y-%m-%d}-${model.ts_flag}-${model.ts_flag2}-rotatem/${latent.gamma}/seed-${latent.seed}/${model.m_step_init}/${now:%H-%M-%S}'
                # for init in empirical-init 
                # for init in empirical-init true-init oracle-init
                for init in oracle-init # true-init 
                do 
                    python scripts/hydra_run_gaussian_rank1_model.py latent.seed=$seed obs.seed=$seed \
                    latent.gamma=$gamma latent.L=$L model.maxiter=10 'model.support=[9,9]' \
                    model.ts_flag2=gd model.ts_flag=$ts_flag model.emiters=$iters \
