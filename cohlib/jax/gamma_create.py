...
def k2_full_testri(flow, fhigh, scalep_target, scalep_offtarget):
    N = 500
    K = 2
    nonzero_freqs = jnp.arange(flow,fhigh+1)
    Nnz = nonzero_freqs.size
    ...
    gamma_target = jnp.array([[5,1],[1,5]],dtype=complex) + jnp.array([[0,-3],[3,0]],dtype=complex)*1j
    ...
    save_path = os.path.join(gamma_root(), f'k2-full{flow}-{fhigh}-10-{scalep_target}-{scalep_offtarget}_testri.pickle')
    save_dict = {'gamma': gamma_full, 'freqs': freqs, 'nonzero_inds': nonzero_inds, 'target_inds': target_inds}
...
def k2_flat(flow, fhigh, scale_power):
    ...
    save_path = os.path.join(gamma_root(), f'k2-flat{flow}-{fhigh}-{scale_power}.pickle')
    save_dict = {'gamma': gamma_full, 'freqs': freqs, 'nonzero_inds': nonzero_inds, 'target_inds': target_inds}
