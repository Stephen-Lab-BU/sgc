...
from cohlib.alg.laplace_sgc import TrialData, SpikeTrialDeltaReLUPoisson

from pathlib import Path
from omegaconf import OmegaConf

def load_results(paths, alpha_sel, ts_flag=False, **kwargs):
...
class OldOptim():
    def __init__(self, data, gamma_inv, params, obs_type, track=False):
        ...
        obs_objs = [SpikeTrialDeltaReLUPoisson(data[None,:,i], params) for i in range(self.K)] 
        if self.mod_to_fix is True:
            ...
            gamma_inv_oldformat = construct_Gamma_full_real(self.gamma_inv[nz,:,:], 
                                    self.K, self.num_J_vars, invert=False)
        trial_obj = TrialData(obs_objs, gamma_inv_oldformat, self.Wv, self.params, 'poisson-relu-delta', "Newton")
    ...
    def eval_cost(self, zs):
        ...
        vs_flat = np.array(jnp.concatenate([vs[:,k] for k in range(self.K)]))
        ...
        return jnp.array(cost_real), jnp.array(grad), jnp.array(hess), jnp.array(hess_full_real)
    ...
    def run_e_step(self, zs_init, num_iters):
        ...
        if self.track is True:
            ...
            result = OptimResult(zs_est, hess)
        self.result = result
        vs_est = conv_mu_old_c2r(zs_est, self.K)
...
class JvOExp():
    def __init__(self, obs, gamma_inv, params, obs_type, method, track_optim=False, track_em=False, track_realrep=False, decon_mod=False):
    ...
    def eval_cost(self, trial, zs=None):
        ...
        optim = self.optimizer(trial_data, self.gamma_inv, self.params, self.obs_type)
    ...
    def e_step(self, num_iters, zs_init=None):
        ...
        for trial in tqdm(range(L)):
            ...
            optim = self.optimizer(trial_data, self.gamma_inv, self.params, self.obs_type, self.track_optim)
