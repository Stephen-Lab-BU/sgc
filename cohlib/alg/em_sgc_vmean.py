...
from cohlib.alg.laplace_sgc_vmean import TrialData, SpikeTrialDeltaLogPoisson, SpikeTrialDeltaReLUPoisson, SpikeTrialDeltaIDPoisson, SpikeTrialBernoulli
    ...
    for m in range(num_tapers):
        ...
        for r in range(num_em_iters):
            ...
            for l in range(L):
                ...
                trial = get_trial_obj(
                    data, l, W, phi_prev, Gamma_prev_inv, params, taper=taper, obs_model=obs_model, optim_type=optim_type,
                )
            ...
            if rho is None:
                if mu_only_update is True:
                    ...
                    Gamma_update_complex = update_Gamma_complex(
                        mus, Ups_invs, phi_prev, K, num_J_vars
                    )
            else:
...
def get_trial_obj(data, l, W, phi_prev, Gamma_inv_prev, params, taper, obs_model, optim_type):
    """
    data is list of spike data (trial x neurons x time)
    """
    if obs_model == "poisson-relu-delta":
...
def update_Gamma_complex(mus, Ups_invs, phi_prev, K, num_J_vars):
    """
    M-Step
