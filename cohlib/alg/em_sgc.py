...
def fit_sgc_model(data, W, inits, tapers, num_em_iters=10, max_approx_iters=10, dc=True, track=False):
    # safety / params
    ...
    for m in range(num_tapers):
        ...
        for r in range(num_em_iters):
            ...
            for l in range(L):
                ...
                trial = get_trial_obj(data, l, W, Gamma_prev_inv, taper=taper)
...
def get_trial_obj(data, l, W, Gamma_inv_prev, taper):
    """
    data is list of spike data (trial x neurons x time)
    """
    trial_data = [group_data[l,:,:] for group_data in data]
    spike_objs = [SpikeTrial(data, taper) for data in trial_data]
